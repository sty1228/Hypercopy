...
export default function ExplorePage() {
  ...
  useEffect(() => {
    ...
    getTokenSentiment(30).then(d => setSentiment(d)).catch(() => setSentiment([])).finally(() => setSentimentLoading(false));

    setTopLoading(true);
    leaderboard("30d").then(d => {
      const list = d.map((item, i) => ({ ...item, rank: i + 1 }));
      setTopTraders(list);
    }).catch(() => setTopTraders([])).finally(() => setTopLoading(false));

    setRisingLoading(true);
    getRisingTraders(6).then(d => setRising(d)).catch(() => setRising([])).finally(() => setRisingLoading(false));
  }, []);

  useEffect(() => {
    if (tab !== "mytraders") return;
    setFollowsLoading(true);
    getFollowedTraders("30d").then(d => setFollows(d)).catch(() => setFollows([])).finally(() => setFollowsLoading(false));
  }, [tab]);
  ...
  const handleSearch = () => {
    if (searchText.trim()) router.push(`/copyTrading?search=${encodeURIComponent(searchText.trim())}`);
  };
  ...
  const categories = [
    { icon: <Crosshair size={16} className="text-teal-400" />, label: "High WR", desc: ">75% accuracy", color: "#2dd4bf" },
    { icon: <Diamond size={16} className="text-indigo-400" />, label: "Holders", desc: "Long-term", color: "#6366f1" },
    { icon: <Zap size={16} className="text-amber-400" />, label: "Scalpers", desc: "Quick trades", color: "#f59e0b" },
    { icon: <Anchor size={16} className="text-pink-400" />, label: "Whales", desc: "Big positions", color: "#ec4899" },
    { icon: <Compass size={16} className="text-purple-400" />, label: "Macro", desc: "Big picture", color: "#8b5cf6" },
    { icon: <Sparkles size={16} className="text-emerald-400" />, label: "New", desc: "< 30 days", color: "#34d399" },
  ];
      ...
      {tab === "discover" && (
        <div className="relative z-10 pb-24">
          ...
          <div className="px-4 mb-4 fade-in fade-in-1">
            ...
            />
            {sentimentLoading ? <LoadingBlock /> : sentiment.length === 0 ? <EmptyBlock text="No signal data available yet" /> : (
              <div className="grid grid-cols-2 gap-2">
                {sentiment.slice(0, 8).map((tk) => (
                  <div key={tk.ticker} className="rounded-xl p-2.5 relative overflow-hidden" style={{ background: cardBg, border: cardBorder }}>
                    ...
                    <div className="flex items-center justify-between mt-1.5 pt-1.5" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                      <StatPill icon={<Activity size={7} />} text={`${tk.total_signals} signals`} />
          ...
          <div className="px-4 mb-4 fade-in fade-in-2">
            ...
            />
            {topLoading ? <LoadingBlock /> : topTraders.length === 0 ? <EmptyBlock text="No traders found" /> : (
                    ...
                    <div key={t.x_handle} onClick={() => goTrader(t.x_handle)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <StatPill icon={<Target size={7} />} text={`${wr.toFixed(0)}%`} />
                          <StatPill icon={<BarChart3 size={7} />} text={`${t.total_signals || t.total_tweets} signals`} />
                          {(t.copiers ?? 0) > 0 && <StatPill icon={<Users size={7} />} text={`${t.copiers}`} />}
                        </div>
                      </div>
      ...
      {tab === "mytraders" && (
        <div className="relative z-10 px-4 pb-24">
          ...
          {followsLoading ? (
            ...
            if (list.length === 0) {
              return (
                <div className="rounded-xl p-8 flex flex-col items-center text-center" style={{ background: cardBg, border: cardBorder }}>
                    ...
                    <div key={f.id} onClick={() => goTrader(f.trader_username)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <StatPill icon={<Target size={7} />} text={`${wr.toFixed(0)}% WR`} />
                          <StatPill icon={<BarChart3 size={7} />} text={`${f.total_signals} signals`} />
                        </div>
                      </div>
                      <div className="text-right shrink-0">
