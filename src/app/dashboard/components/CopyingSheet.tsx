  ...
  if (source === "mixed") {
    return (
      <span className="flex gap-0.5">
        <span className="px-1 py-0.5 rounded-l text-[8px] font-semibold"
          style={{ background:"rgba(45,212,191,0.15)",color:"#2dd4bf",border:"1px solid rgba(45,212,191,0.25)" }}>C</span>
        <span className="px-1 py-0.5 rounded-r text-[8px] font-semibold"
          style={{ background:"rgba(245,158,11,0.15)",color:"#f59e0b",border:"1px solid rgba(245,158,11,0.25)" }}>CT</span>
      </span>
    );
  }
  if (is_counter) {
...
const TraderRow = ({
  trader, rank, delay, onClick,
}: {
  trader: TraderData; rank: number | null; delay: number; onClick: () => void;
}) => {
  const [vis, setVis] = useState(false);
  useEffect(() => { const t = setTimeout(() => setVis(true), delay); return () => clearTimeout(t); }, [delay]);
  const up = trader.pnl >= 0;
  const { c1, c2 } = getAvatarColor(trader.name);
  const borderColor = rank === 1 ? "#FFD700" : rank === 2 ? "#C0C0C0" : rank === 3 ? "#CD7F32" : "rgba(255,255,255,0.08)";

  return (
    <div
      className="flex items-center justify-between py-3.5 px-4 cursor-pointer"
      style={{
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-shrink-0">
          <div className="flex items-center justify-center rounded-full text-white font-bold"
            style={{ width:40,height:40,fontSize:15,background:`linear-gradient(135deg,${c1},${c2})`,border:`2px solid ${borderColor}` }}>
...
const SummaryBar = ({ traders }: { traders: TraderData[] }) => {
  ...
  return (
    <div className="mx-4 mb-3 rounded-xl px-4 py-2.5 flex items-center justify-between"
      ...
      <span className="text-xs" style={{ color:"rgba(255,255,255,0.4)" }}>Total Copy PnL</span>
      <span className="text-sm font-bold" style={{ color: up ? "#22c55e" : "#ef4444" }}>
        {up ? "+" : "−"}${Math.abs(total).toLocaleString(undefined, { minimumFractionDigits:2,maximumFractionDigits:2 })}
      </span>
    </div>
  );
};
...
// ---------- Main Component ----------
const CopyingSheet = ({ mode, onClose, userBalance = 0, onDepositClick }: CopyingSheetProps) => {
  ...
  useEffect(() => {
    if (mode === "copying") fetchCopying();
    else { setLoading(false); setTraders([]); }
  }, [mode]);
  ...
  const fetchCopying = async () => {
    ...
    try {
      ...
      const list: TraderData[] = (followRes || []).map((item) => {
        const username = item.trader_username || "";
        const pnlData = pnlMap.get(username);
        return {
          ...
          trade_count: pnlData ? pnlData.trade_count : 0,
          open_count: pnlData ? pnlData.open_count : 0,
          source: pnlData ? pnlData.source : (item.is_counter_trading ? "counter" : "copy"),
        };
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={handleClose}>
      <div className="absolute inset-0 transition-opacity duration-300"
        style={{ background:"rgba(0,0,0,0.75)",backdropFilter:"blur(10px)",opacity:closing?0:1 }} />
      <div
          ...
          <div className="mx-4 mb-6 rounded-2xl overflow-hidden"
            ...
            {sorted.map((t, i) => (
              <TraderRow
                ...
                delay={i * 50}
                onClick={() => handleTraderClick(t.username)}
              />
            ))}
          </div>
        )}
