...
import { recordWithdraw } from "@/service";
...
interface WithdrawSheetProps {
  isOpen: boolean;
  onClose: () => void;
  availableBalance: number; // HyperCopy BalanceSnapshot 记录的余额
...
export default function WithdrawSheet({ isOpen, onClose, availableBalance, onSuccess }: WithdrawSheetProps) {
  ...
  // HL 真实总余额
  const [hlTotalBalance, setHlTotalBalance] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 打开时读取 HL 真实余额
      if (infoClient && wallet?.address) {
        setLoadingBalance(true);
        getPerpsBalance({ exchClient: infoClient, walletAddress: wallet.address })
          .then(res => {
            const bal = parseFloat(res?.marginSummary?.accountValue ?? "0");
            setHlTotalBalance(bal);
          })
          .catch(() => setHlTotalBalance(null))
          .finally(() => setLoadingBalance(false));
      }
    } else {
      setSheetVisible(false);
      setHlTotalBalance(null);
    }
  }, [isOpen, wallet?.address]);

  // 实际可提金额 = min(HyperCopy记录余额, HL真实余额)
  // 防止提走超出平台管理的部分，也防止余额不足报错
  const effectiveMax = hlTotalBalance !== null
    ? Math.min(availableBalance, hlTotalBalance)
    : availableBalance;
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      await (mainExchClient as any).withdraw3({
        ...
        amount: withdrawAmount.toFixed(2), // HL 要求字符串，保留 2 位小数
      });
          ...
          <div className="px-5 pb-6">
                ...
                >
                  ...
                  {loadingBalance ? (
                    ...
                    <div className="text-right">
                      ...
                      {/* 如果 HL 余额 < HyperCopy 记录余额，提示用户 */}
                      {hlTotalBalance !== null && hlTotalBalance < availableBalance && (
                        <p className="text-[9px] text-yellow-400 mt-0.5">
                          Adjusted due to trading PnL (recorded: ${availableBalance.toFixed(2)})
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div>
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${parsedAmount > effectiveMaxRounded ? "rgba(239,68,68,0.4)" : "rgba(255,255,255,0.08)"}`,
                    }}
                  >
            ...
            {(step === "signing" || step === "processing") && (
              <div className="py-8 flex flex-col items-center text-center">
                ...
                <h3 className="text-sm font-bold text-white mb-1">
                  {step === "signing" ? "Sign Withdrawal..." : "Processing..."}
                </h3>
                <p className="text-[11px] text-gray-400 max-w-[240px]">
                  {step === "signing" ? "Please confirm the withdrawal in your wallet" : "Withdrawal is being processed"}
                </p>
                <div className="flex items-center gap-2 mt-6">
                  {["Sign", "Process"].map((s, i) => {
            ...
            {step === "success" && (
              <div className="py-8 flex flex-col items-center text-center">
                ...
                <p className="text-[11px] text-gray-400 mb-1">${parsedAmount.toFixed(2)} USDC will arrive in your Arbitrum wallet</p>
