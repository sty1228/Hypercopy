...
export default function CopyTrading() {
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden" style={{ background: "linear-gradient(180deg, #0a0f14 0%, #080d10 100%)" }}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {filters.map((filter) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all duration-300 flex items-center gap-1.5 ${activeFilter === filter.key ? "text-white" : "text-gray-400"}`}
              style={{
      ...
      {/* Section Header */}
      <div className="relative z-10 px-4 mb-2 flex items-center justify-between">
        <span className="text-white text-sm font-semibold">Top Traders</span>
        <div className="flex gap-1">
          {["24h", "7d", "30d"].map((t) => (
            <button
              key={t}
              onClick={() => setTimeFilter(t)}
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all duration-300 ${timeFilter === t ? "text-white bg-white/10" : "text-gray-500"}`}
            >
              {t}
            </button>
          ))}
